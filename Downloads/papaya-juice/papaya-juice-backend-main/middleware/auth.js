...
// Hash factice pour constant-time compare (anti timing-attack)
const DUMMY_HASH = '$2b$10$dummyhashpaddingthatisnotreal.invalidXXXXXXXXXXXXXXXXX';
...
router.post('/login', async (req, res) => {
  ...
  // Anti timing-attack: même si user introuvable, on fait quand même un bcrypt.compare
  // pour que le temps de réponse soit identique (évite l'énumération d'emails)
  if (error || !data || data.length === 0) {
    await bcrypt.compare(password, DUMMY_HASH); // constant-time — résultat ignoré
  ...
  if (emp.password && emp.password.startsWith('$2')) {
    ...
    // Legacy plain text — constant-time dummy compare d'abord
